...
namespace ChunChen_CRM.App_Start
{
    ...
    public class AutoFacConfig
    {
        public static void Register()
        {
            ...
            Assembly controllerAss = Assembly.Load("ChunChen-CRM");

            //2.2 告诉AutoFac控制器工厂，控制器的创建从controllerAss中查找
            //（注意：RegisterControllers()方法是一个可变参数，如果你的控制器类的创建需要去多个程序集中查找的话，
            //那么我们就再用Assembly controllerBss=Assembly.Load("需要的程序集名")加载需要的程序集，
            ...
            //（其实就是创建数据仓储层与业务逻辑层这两个程序集中所有类的对象实例，然后以其接口的形式保存到AutoFac容器内存中，
            //当然如果有需要也可以创建其他程序集的所有类的对象实例，这个只需要我们指定就可以了）  
